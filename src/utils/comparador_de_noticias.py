...
class NewsAnalyzer:
    def __init__(self, database="meneame"):
        load_dotenv()
    ...
    def comparar_plotly(self, entidad1, entidad2, tipo='noticia'):
        ...
        if tipo == 'noticia':
            df = self.get_data(news_ids=[entidad1, entidad2])
            if df.empty:
            ...
            data1 = self.normalize_values(noticia1.iloc[0][variables], max_values, variables)
            data2 = self.normalize_values(noticia2.iloc[0][variables], max_values, variables)
